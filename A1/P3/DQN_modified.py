...
if __name__ == "__main__":
    ...
    tgt_batch_update = [1, 10, 50, 100]
    train_epochs_update = [1, 5, 10, 50]
    colours = ['b', 'g', 'm', 'k']
    ...
    for part in [1, 2, 3]:
        ...
        for i in range(4):
            curves = []
            if part == 1:
                TARGET_UPDATE_FREQ = tgt_batch_update[i]
            elif part == 2:
                TARGET_UPDATE_FREQ = 10
                MINIBATCH_SIZE =  tgt_batch_update[i]
            elif part == 3:
                MINIBATCH_SIZE = 10
                TRAIN_EPOCHS = train_epochs_update[i]
            ...
            if part == 1: 
                plot_arrays(curves, colours[i], f'Target Update Frequency: {tgt_batch_update[i]}')
            elif part == 2:
                plot_arrays(curves, colours[i], f'Minibatch Size: {tgt_batch_update[i]}')
            elif part == 3:
                plot_arrays(curves, colours[i], f'Train Epochs: {train_epochs_update[i]}')

...
def update_networks(epi, buf, Z, Zt, OPT):
    ...
    with torch.no_grad():
        probs_prime = torch.nn.Softmax(dim=2)(Zt(S_prime).view(MINIBATCH_SIZE, ACT_N, ATOMS)) # Ensure policy selection is arranged the same way!
        ...
        # = probs_prime[i][a_greedy[i]]*(u_index[i] - index[i])

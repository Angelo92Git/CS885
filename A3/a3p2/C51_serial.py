...
##This implementation does not work well and is very slow. Included for reference.
...
def policy(env, obs):
    ...
    if np.random.rand() < EPSILON:
        ...
        with torch.no_grad():
            probs = torch.nn.Softmax(dim=-1)(Z(obs).view(2, -1))
...
def update_networks(epi, buf, Z, Zt, OPT):
    ...
    for i in range(MINIBATCH_SIZE):
        ...
        with torch.no_grad():
            probs_prime = torch.nn.Softmax(dim=-1)(Zt(S_prime[i]).view(2, -1))
        ...
        OPT.zero_grad()
        z_out = torch.nn.Softmax(dim=-1)(Z(S[i]).view(2, -1))[A[i]]
        loss = -(p*z_out.log()).sum()
        loss.backward()
        OPT.step()
